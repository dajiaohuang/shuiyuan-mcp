...
5. Execute writes with the preview-confirm flow: first call for preview, then call again with `confirm_send=true` and `preview_token`.
6. After writing, read the topic back and verify title/body/category rendered correctly.
7. If Chinese text renders as `?`, fix the affected title/post immediately using a UTF-8-safe path.
...
- For any write tool, first generate preview and ask user to modify or confirm send.
- Only execute when user explicitly confirms and the tool call includes `confirm_send=true` plus the latest `preview_token`.
